...
* [Light Overlay](https://modrinth.com/mod/light-overlay) adds a toggleable block overlay (<kbd>F7</kbd> by default) that shows whether mobs can spawn on a given block according to its light level. A red cross means mobs can spawn any time, yellow means mobs can spawn at night, and blue indicates that mobs can only spawn if there's a mob spawner nearby.
...
* [Quick Connect Button](https://modrinth.com/mod/quickconnectbutton) adds a "Connect" button to the main menu that will connect you directly to the Go Pound Soul Sand server. No more wasting precious seconds clicking through the Multiplayer menu!
